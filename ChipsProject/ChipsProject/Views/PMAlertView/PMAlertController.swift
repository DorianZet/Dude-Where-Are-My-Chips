...
@objc open class PMAlertController: UIViewController {
    ...
    @objc open func addAction(_ alertAction: PMAlertAction){
        ...
        if alertActionStackView.arrangedSubviews.count > 2 || hasTextFieldAdded(){
            alertActionStackViewHeightConstraint.constant = ALERT_STACK_VIEW_HEIGHT * CGFloat(alertActionStackView.arrangedSubviews.count) // pokombinuj cos z zaleznoscia stackview height od ilosci arrangedSubviews ALBO porownaj wysokosc heightu alertView z 1/2 frame.height - jeli jest wyzsza, to zmniejsz height do jakiejs bezpiecznej wielkosci, np. 30-35.
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        ...
        // kontynuuj tu
        var newContentViewFrameY = alertView.frame.maxY - endKeyBoardFrame + 10 // putting "10" in so that keyboard doesn't touch the alertview.
...
//        guard let endKeyBoardFrameHeight = (userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.height else {return}
//        if alertView.frame.height > super.view.frame.height - endKeyBoardFrameHeight + 10 {
//            let heightToSet: Float = Float(super.view.frame.height - endKeyBoardFrameHeight - 10 - headerViewHeightConstraint.constant)
//            alertActionStackViewHeightConstraint.constant = CGFloat(heightToSet)
//            alertContentStackViewTopConstraint.constant = 0 ???
//        }
        // wczesniej to bylo tu gdzie jest 'kontynuuj tu'
//        var newContentViewFrameY = alertView.frame.maxY - endKeyBoardFrame + 10 // putting "10" in so that keyboard doesn't touch the alertview.
//        if newContentViewFrameY < 0 {
//            newContentViewFrameY = 0
//        }
